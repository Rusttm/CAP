...
class ConnPgsqlSocSrvMainClass(PgsqlMainClass):
    ...
    def start_socket_client(self):
        try:
            ...
            self.logger.info(f"Socket client {PgsqlMainClass} was interrupted by admin")
...
if __name__ == '__main__':
    client_name = "pgsql"
    connector = ConnPgsqlSocSrvMainClass(client_name=client_name)
    # Thread(target=connector.start_socket_client, args=[]).start()
    # connector.start_socket_client()
    connector.outgoing_msg_queue.append({'from': client_name, 'to': 'main',
