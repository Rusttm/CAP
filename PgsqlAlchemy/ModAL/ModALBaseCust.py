...
class ModALBaseCust(Base):
    ...
    accounts = Column(MutableList.as_mutable(ARRAY(JSONB)),
                      comment='Массив счетов Контрагентов. Подробнее тут Обязательное при ответе Expand')
    actualAddress = Column(String(255), comment='Фактический адрес Контрагента')
    actualAddressFull = Column(JSONB,
                               comment='Фактический адрес Контрагента с детализацией по отдельным полям. Подробнее тут')
    ...
    companyType = Column(String(255),
                         comment='Тип Контрагента. В зависимости от значения данного поля набор выводимых реквизитов контрагента может меняться. Подробнее тут Обязательное при ответе')
    contactpersons = Column(MutableList.as_mutable(ARRAY(JSONB)),
                            comment='Массив контактных лиц фирмы Контрагента. Подробнее тут Expand')
    ...
    discounts = Column(MutableList.as_mutable(ARRAY(JSONB)),
                       comment='Массив скидок Контрагента. Массив может содержать персональные и накопительные скидки. Персональная скидка выводится, если хотя бы раз изменялся процент скидки для контрагента, значение будет указано в поле personalDiscount')
    ...
    files = Column(MutableList.as_mutable(ARRAY(JSONB)),
                   comment='Метаданные массива Файлов (Максимальное количество файлов - 100) Обязательное при ответе Expand')
    group = Column(JSONB, comment='Отдел сотрудника Обязательное при ответе Expand')
    id = Column(Uuid, unique=True, comment='ID Контрагента Обязательное при ответе Только для чтения')
    meta = Column(JSONB, unique=True, comment='Метаданные Контрагента Обязательное при ответе')
    name = Column(String(255), unique=True,
                  comment='Наименование Контрагента Обязательное при ответе Необходимо при создании')
    notes = Column(MutableList.as_mutable(ARRAY(JSONB)), comment='Массив событий Контрагента. Подробнее тут Expand')
    ...
    updated = Column(DateTime,
                     comment='Момент последнего обновления Контрагента Обязательное при ответе Только для чтения')
    ...
    legalFirstName = Column(String(255),
                            comment='Имя для Контрагента типа [Индивидуальный предприниматель, Физическое лицо]. Игнорируется для Контрагентов типа [Юридическое лицо]')
    legalLastName = Column(String(255),
                           comment='Фамилия для Контрагента типа [Индивидуальный предприниматель, Физическое лицо]. Игнорируется для Контрагентов типа [Юридическое лицо]')
    legalMiddleName = Column(String(255),
                             comment='Отчество для Контрагента типа [Индивидуальный предприниматель, Физическое лицо]. Игнорируется для Контрагентов типа [Юридическое лицо]')
    legalTitle = Column(String(4096),
                        comment='Полное наименование для Контрагента типа [Юридическое лицо]. Игнорируется для Контрагентов типа [Индивидуальный предприниматель, Физическое лицо], если передано одно из значений для ФИО и формируется автоматически на основе получаемых ФИО Контрагента')
