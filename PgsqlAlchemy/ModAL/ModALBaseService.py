...
class ModALBaseService(Base):
	__tablename__ = 'pgsql_service'
	__table_args__ = (UniqueConstraint('event_id', name='unique_key_event_id'),)
	...
	event_id = Column(Uuid, unique=True, nullable=False, comment='ID сервисной записи')
	event_active = Column(Boolean, comment='требуется ли реакция на сообщение? активно ли оно?')
	event_level = Column(BigInteger, comment='уровень события: как в логере 0-notset, 10-debug, 20-info, 30-warning, 40-error, 50-critical')
	...
	event_to = Column(String(255), comment='для кого предназначено событие?')
